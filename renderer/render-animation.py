...
def infer_anistart(dsn, prefix, bbox):
    ...
    sql = "SELECT MIN(valid_from) FROM hist_point WHERE geom && ST_Transform(ST_SetSRID(ST_MakeBox2D(ST_Point(%f, %f), ST_Point(%f, %f)), 4326), 3857)" % (bbox[0], bbox[1], bbox[2], bbox[3])
